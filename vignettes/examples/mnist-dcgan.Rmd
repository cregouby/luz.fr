---
title: "DCGAN"
desc: "Réseau neuronal profond convolutionnel génératif et discriminant pour la génération d'images."
...
generator <- nn_module(
  ...
  initialize = function(latent_dim, out_channels) {
    ...
    self$main$apply(init_weights) # initialisation personalisée des poids
  },
...
dcgan <- torch::nn_module(
  ...
  loss = function(input, ...) {
    ...
    # creation du vecteur de réponse
    y_real <- torch_ones(batch_size, device = device)
    y_fake <- torch_zeros(batch_size, device = device)

    # calcul avec des fonctions de coûts différentes suivant l'optimiseur
